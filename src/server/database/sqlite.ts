...
const defaultNftIptablesPostUp =
  '/usr/sbin/iptables-nft -w -t nat -C POSTROUTING -s {{ipv4Cidr}} -j MASQUERADE 2>/dev/null || /usr/sbin/iptables-nft -w -t nat -A POSTROUTING -s {{ipv4Cidr}} -j MASQUERADE; ' +
  '/usr/sbin/iptables-nft -w -C INPUT -p udp -m udp --dport {{port}} -j ACCEPT 2>/dev/null || /usr/sbin/iptables-nft -w -A INPUT -p udp -m udp --dport {{port}} -j ACCEPT; ' +
  '/usr/sbin/iptables-nft -w -C FORWARD -i {{interface}} -j ACCEPT 2>/dev/null || /usr/sbin/iptables-nft -w -A FORWARD -i {{interface}} -j ACCEPT; ' +
  '/usr/sbin/iptables-nft -w -C FORWARD -o {{interface}} -j ACCEPT 2>/dev/null || /usr/sbin/iptables-nft -w -A FORWARD -o {{interface}} -j ACCEPT; ' +
  '/usr/sbin/ip6tables-nft -w -t nat -C POSTROUTING -s {{ipv6Cidr}} -j MASQUERADE 2>/dev/null || /usr/sbin/ip6tables-nft -w -t nat -A POSTROUTING -s {{ipv6Cidr}} -j MASQUERADE; ' +
  '/usr/sbin/ip6tables-nft -w -C INPUT -p udp -m udp --dport {{port}} -j ACCEPT 2>/dev/null || /usr/sbin/ip6tables-nft -w -A INPUT -p udp -m udp --dport {{port}} -j ACCEPT; ' +
  '/usr/sbin/ip6tables-nft -w -C FORWARD -i {{interface}} -j ACCEPT 2>/dev/null || /usr/sbin/ip6tables-nft -w -A FORWARD -i {{interface}} -j ACCEPT; ' +
  '/usr/sbin/ip6tables-nft -w -C FORWARD -o {{interface}} -j ACCEPT 2>/dev/null || /usr/sbin/ip6tables-nft -w -A FORWARD -o {{interface}} -j ACCEPT;';

const defaultNftIptablesPostDown =
  'while /usr/sbin/iptables-nft -w -t nat -D POSTROUTING -s {{ipv4Cidr}} -j MASQUERADE 2>/dev/null; do :; done; ' +
  'while /usr/sbin/iptables-nft -w -D INPUT -p udp -m udp --dport {{port}} -j ACCEPT 2>/dev/null; do :; done; ' +
  'while /usr/sbin/iptables-nft -w -D FORWARD -i {{interface}} -j ACCEPT 2>/dev/null; do :; done; ' +
  'while /usr/sbin/iptables-nft -w -D FORWARD -o {{interface}} -j ACCEPT 2>/dev/null; do :; done; ' +
  'while /usr/sbin/ip6tables-nft -w -t nat -D POSTROUTING -s {{ipv6Cidr}} -j MASQUERADE 2>/dev/null; do :; done; ' +
  'while /usr/sbin/ip6tables-nft -w -D INPUT -p udp -m udp --dport {{port}} -j ACCEPT 2>/dev/null; do :; done; ' +
  'while /usr/sbin/ip6tables-nft -w -D FORWARD -i {{interface}} -j ACCEPT 2>/dev/null; do :; done; ' +
  'while /usr/sbin/ip6tables-nft -w -D FORWARD -o {{interface}} -j ACCEPT 2>/dev/null; do :; done;';
