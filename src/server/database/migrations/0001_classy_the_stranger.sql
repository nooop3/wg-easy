...
VALUES (
  'wg0',
  '',
  '/usr/sbin/iptables-nft -w -t nat -C POSTROUTING --source {{ipv4Cidr}} --out-interface {{device}} -j MASQUERADE 2>/dev/null || /usr/sbin/iptables-nft -w -t nat -A POSTROUTING --source {{ipv4Cidr}} --out-interface {{device}} -j MASQUERADE; /usr/sbin/iptables-nft -w -C INPUT --protocol udp --match udp --dport {{port}} -j ACCEPT 2>/dev/null || /usr/sbin/iptables-nft -w -A INPUT --protocol udp --match udp --dport {{port}} -j ACCEPT; /usr/sbin/iptables-nft -w -C FORWARD --in-interface {{interface}} -j ACCEPT 2>/dev/null || /usr/sbin/iptables-nft -w -A FORWARD --in-interface {{interface}} -j ACCEPT; /usr/sbin/iptables-nft -w -C FORWARD --out-interface {{interface}} -j ACCEPT 2>/dev/null || /usr/sbin/iptables-nft -w -A FORWARD --out-interface {{interface}} -j ACCEPT; /usr/sbin/ip6tables-nft -w -t nat -C POSTROUTING --source {{ipv6Cidr}} --out-interface {{device}} -j MASQUERADE 2>/dev/null || /usr/sbin/ip6tables-nft -w -t nat -A POSTROUTING --source {{ipv6Cidr}} --out-interface {{device}} -j MASQUERADE; /usr/sbin/ip6tables-nft -w -C INPUT --protocol udp --match udp --dport {{port}} -j ACCEPT 2>/dev/null || /usr/sbin/ip6tables-nft -w -A INPUT --protocol udp --match udp --dport {{port}} -j ACCEPT; /usr/sbin/ip6tables-nft -w -C FORWARD --in-interface {{interface}} -j ACCEPT 2>/dev/null || /usr/sbin/ip6tables-nft -w -A FORWARD --in-interface {{interface}} -j ACCEPT; /usr/sbin/ip6tables-nft -w -C FORWARD --out-interface {{interface}} -j ACCEPT 2>/dev/null || /usr/sbin/ip6tables-nft -w -A FORWARD --out-interface {{interface}} -j ACCEPT;',
  '',
  'while /usr/sbin/iptables-nft -w -t nat -D POSTROUTING --source {{ipv4Cidr}} --out-interface {{device}} -j MASQUERADE 2>/dev/null; do :; done; while /usr/sbin/iptables-nft -w -D INPUT --protocol udp --match udp --dport {{port}} -j ACCEPT 2>/dev/null; do :; done; while /usr/sbin/iptables-nft -w -D FORWARD --in-interface {{interface}} -j ACCEPT 2>/dev/null; do :; done; while /usr/sbin/iptables-nft -w -D FORWARD --out-interface {{interface}} -j ACCEPT 2>/dev/null; do :; done; while /usr/sbin/ip6tables-nft -w -t nat -D POSTROUTING --source {{ipv6Cidr}} --out-interface {{device}} -j MASQUERADE 2>/dev/null; do :; done; while /usr/sbin/ip6tables-nft -w -D INPUT --protocol udp --match udp --dport {{port}} -j ACCEPT 2>/dev/null; do :; done; while /usr/sbin/ip6tables-nft -w -D FORWARD --in-interface {{interface}} -j ACCEPT 2>/dev/null; do :; done; while /usr/sbin/ip6tables-nft -w -D FORWARD --out-interface {{interface}} -j ACCEPT 2>/dev/null; do :; done;'
);
--> statement-breakpoint
